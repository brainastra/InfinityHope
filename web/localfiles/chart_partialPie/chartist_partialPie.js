	...
	if(data.type==='label')
      {
        ...
        data.element.attr({
        style: 'font-size:'+parseInt(0.05*wid)+'px;'
      });
      }
...
konymp.charts.pie.prototype.UI_createpieCharts = function(lbls,srs,legends,cutout)
{
  ...
  if(document.readyState === "complete")
  {
   ...
   document.body.style.backgroundColor = properties._bgColor;
    this.Updatecss(colors);
      this.UI_createpieCharts(labels,[data],properties._enableLegend,properties._chartCutOut);
    return true;
  }
  else
  {
...
onbodyload = function(){
 if(typeof kony !== "undefined") {
   kony.evaluateJavaScriptInNativeContext("chart_partialPie_defined_global('ready')");
 } else {
  drawCanvasChart();
 }
}.bind(this);
